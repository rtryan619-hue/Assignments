...
# histogram using a list
print("HISTOGRAM USING LIST")
printed_words = []

for word in clean_word_list:
    if word not in printed_words:
        count = clean_word_list.count(word)
        print(word + ":", "*" * count)
...
phone_numbers = re.findall(r'\d{1}-\d{3}-\d{3}-\d{4}', story)
email_addresses = re.findall(r'[\w.-]+@[\w.-]+\.\w+', story)
...
for email in email_addresses:
    ...
    hotmail_list.append(username + "@hotmail.com")
